...
class Synthesizer:
	def load(self, checkpoint_path, hparams, gta=False, model_name='Tacotron',model_version=1):
		...
		open("save_module/quantized_model.tflite", "wb").write(tflite_quantized_model)
		...
		for i, mel in enumerate(mels):
			...
			if log_dir is not None:
				...
				if hparams.predict_linear:
					#save wav (linear -> wav)
					linear_filename = os.path.join(out_dir, 'liner-{}.npy'.format(basenames[i]))
					np.save(linear_filename, linears, allow_pickle=False)
